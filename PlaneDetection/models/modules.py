"""
Implementation of the PEC network in the R class. Other code from:
...
class R(nn.Module):
    def __init__(self, block_channel):
        ...
        # normal and depth
        conv00 = nn.Conv2d(num_features, num_features, kernel_size=5, stride=1, padding=2, bias=False)
        bn00 = nn.BatchNorm2d(num_features)

        conv01 = nn.Conv2d(num_features, num_features, kernel_size=5, stride=1, padding=2, bias=False)
        bn01 = nn.BatchNorm2d(num_features)

        conv02 = nn.Conv2d(num_features, num_features, kernel_size=5, stride=1, padding=2, bias=True)
        bn02 = nn.BatchNorm2d(num_features)

        conv03 = nn.Conv2d(num_features, 3, kernel_size=5, stride=1, padding=2, bias=True)

        self.normal_depth = nn.Sequential(conv00, bn00, relu,
                                          conv01, bn01, relu,
                                          conv02, bn02, relu,
                                          conv03)

        # embedding
        conv10 = nn.Conv2d(num_features, num_features, kernel_size=5, stride=1, padding=2, bias=False)
        bn10 = nn.BatchNorm2d(num_features)

        conv11 = nn.Conv2d(num_features, num_features, kernel_size=5, stride=1, padding=2, bias=False)
        bn11 = nn.BatchNorm2d(num_features)

        conv12 = nn.Conv2d(num_features, num_features, kernel_size=5, stride=1, padding=2, bias=False)
        bn12 = nn.BatchNorm2d(num_features)

        conv13 = nn.Conv2d(num_features, num_features, kernel_size=3, stride=1, padding=1, bias=False)
        bn13 = nn.BatchNorm2d(num_features)

        conv14 = nn.Conv2d(num_features, 8, kernel_size=(1, 1), stride=1, padding=0, bias=False)

        self.embedding = nn.Sequential(conv10, bn10, relu,
                                       conv11, bn11, relu,
                                       conv12, bn12, relu,
                                       conv13, bn13, relu,
                                       conv14)
    ...
    def forward(self, x):
        ...
        normal_depth = self.normal_depth(x)

        embeddings = self.embedding(x)

        return normal_depth, embeddings
