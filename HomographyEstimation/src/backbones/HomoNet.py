...
class Model(nn.Module):
    ...
    def _forward(self, x):
        ...
        out = self.layer5(out)
        out = self.layer6(out)
        feat = self.layer7(out)
        out = self.layer8(feat)
        return out, feat

    def forward(self, data):

        (e1, e2) = self.patch_keys  # e1 = 'patch_1', e2 = 'patch_2'

        # Oneline
        o1 = self.target_keys[0]  # traget_key = 'pf_hat_12'
        p1 = data[e1]
        p2 = data[e2]
        x12 = torch.cat([p1, p2], axis=1)
        data[o1], feat = self._forward(x12)
