...
def calculate_epe_hpatches(net, val_loader, device, img_size=240):
    ...
    for _, mini_batch in pbar:
        ...
        source_img = mini_batch['patch_1'].to(device)
        target_img = mini_batch['patch_2'].to(device)
        bs, _, _, _ = source_img.shape

        mini_batch['patch_1'] = mini_batch['patch_1'].to(device)
        mini_batch['patch_2'] = mini_batch['patch_2'].to(device)
        # net prediction
        estimates_grid, estimates_grid_ransca = net.predict_corresponding(mini_batch)
        estimates_grid_ransca = estimates_grid_ransca[0]
        flow_est = estimates_grid.reshape(bs, img_size, img_size, 2)
        flow_est_ransca = estimates_grid_ransca.reshape(bs, img_size, img_size, 2)
        flow_est = torch.from_numpy(flow_est).to(device)
        flow_est_ransca = torch.from_numpy(flow_est_ransca).to(device)
        # flow_est = estimates_grid.permute(0, 2, 3, 1).to(device)
        # flow_est = estimates_grid[-1].permute(0, 2, 3, 1).to(device)
        flow_target = mini_batch['target'].to(device)

        # add flow to the grid

        # applying mask
        mask_x_gt = \
            flow_target[:, :, :, 0].ge(0) & flow_target[:, :, :, 0].le(img_size -1)
        mask_y_gt = \
            flow_target[:, :, :, 1].ge(0) & flow_target[:, :, :, 1].le(img_size -1)
        ...
        flow_est_x = flow_est_ransca[:, :, :, 0]
        flow_est_y = flow_est_ransca[:, :, :, 1]
...
def calculate_pck_hpatches(net, val_loader, device, alpha=1, img_size=240):
    ...
    for _, mini_batch in pbar:
        ...
        # network estimates
        estimates_grid, _ = net(source_img, target_img)
